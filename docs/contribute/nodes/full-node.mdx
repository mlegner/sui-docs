...
1.  Start the Sui Full node:
    ...
    ```
1.  Optional: [Publish/subscribe](../../reference/events-api.mdx#subscribe-events) to notifications using JSON-RPC via websocket.

If your setup is successful, your Sui Full node is now connected to the appropriate network.

Your Full node serves the read endpoints of the [Sui JSON-RPC API](../../reference/json-rpc/read-api.mdx) at: `http://127.0.0.1:9000`.
