...
This guide is useful for developers interested in Sui network interactions via API and should be used in conjunction with the [SuiJSON format](../../reference/json-rpc/json-rpc-format.mdx) for aligning JSON inputs with Move Call arguments.

For a similar guide on Sui network interactions via CLI, refer to the [Sui Client CLI](cli/client-cli.mdx) documentation.

Follow the instructions to [install Sui binaries](cli/install-sui.mdx#install-sui-binaries).

## Connect to a Sui network {#connect-sui}

You can connect to a Sui Full node on a Sui network. Follow the guidance in the [Connect to a Sui Network](connect-to-a-network.mdx) topic to start making RPC calls to the Sui network.
...
- [Sui Rust SDK](../../reference/sdk/rust-sdk.mdx), a collection of Rust language JSON-RPC wrapper and crypto utilities.
...
### Invoke Move functions {#invoke-move-functions}
...
function is described in more detail in the [Sui CLI client](cli/client-cli.mdx#calling-move-code) documentation.
...
To learn more about which `args` a Move call accepts, see the [Sui JSON-RPC API](../../reference/json-rpc/read-api.mdx).

### Publish a Move package {#publish-move-package}
